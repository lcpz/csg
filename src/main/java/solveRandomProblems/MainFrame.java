...
public class MainFrame extends JFrame {
	 ...
	 */
	private void main() {
		if (solveCSGBasedOnPredefinedInput_radioButton.isSelected()) { // If we want to run experiments from a
																		// pre-defined input
			...
			if (input == null)
				return; // If "input" is null, then there was an error in the input, and no action is
						// taken

			// Set the name and path of the output folder
			Calendar calendar = Calendar.getInstance();
			SimpleDateFormat simpleDateFormat = new SimpleDateFormat("yyyy-MM-dd (HH-mm-ss)");
			input.outputFolder = "D:/CSGtemp/" + input.numOfAgents + "agents "
	...
	public Input readInputFromGUI() {
		...
		input.numOfAgents = Integer.valueOf(numOfAgents_textField.getText());
		if (input.numOfAgents > 26) {
			JOptionPane.showMessageDialog(null,
					"The number of coalition structures cannot be handled by java for more than 26 agents!", "Alert",
		...
		if (takeAverage_checkBox.isSelected())
			input.numOfRunningTimes = Integer.valueOf(numOfRunningtimes_textField.getText());
		else
			input.numOfRunningTimes = 1;

		if (ipOrdersIntegerPartitionsAscendingly_checkBox.isSelected())
			input.orderIntegerPartitionsAscendingly = true;
		else
			input.orderIntegerPartitionsAscendingly = false;

		input.acceptableRatio = Double.valueOf(ipAcceptableRatio_textField.getText());

		// Read the distribution of coalition values to be generated
		if (uniform_radioButton.isSelected())
			input.valueDistribution = ValueDistribution.UNIFORM;
		else if (normal_radioButton.isSelected())
			input.valueDistribution = ValueDistribution.NORMAL;
		else if (NDCS_radioButton.isSelected())
			input.valueDistribution = ValueDistribution.NDCS;
		else if (beta_radioButton.isSelected())
			input.valueDistribution = ValueDistribution.BETA;
		else if (gamma_radioButton.isSelected())
			input.valueDistribution = ValueDistribution.GAMMA;
		else if (exponential_radioButton.isSelected())
			input.valueDistribution = ValueDistribution.EXPONENTIAL;
		else if (modifiedUniform_radioButton.isSelected())
			input.valueDistribution = ValueDistribution.MODIFIEDUNIFORM;
		else if (modifiedNormal_radioButton.isSelected())
			input.valueDistribution = ValueDistribution.MODIFIEDNORMAL;
		else if (agentBasedUniform_radioButton.isSelected())
			input.valueDistribution = ValueDistribution.AGENTBASEDUNIFORM;
		else if (agentBasedNormal_radioButton.isSelected())
			input.valueDistribution = ValueDistribution.AGENTBASEDNORMAL;

		// Select the algorithm you want to run
		if (runDP_radioButton.isSelected())
			input.solverName = SolverNames.DP;
		else if (runIDP_radioButton.isSelected())
			input.solverName = SolverNames.IDP;
		else if (runODP_radioButton.isSelected())
			input.solverName = SolverNames.ODP;
		else if (runIP_radioButton.isSelected())
			input.solverName = SolverNames.IP;
		else if (runODPIP_radioButton.isSelected())
			input.solverName = SolverNames.ODPIP;
		else if (runInclusionExclusion_radioButton.isSelected())
			input.solverName = SolverNames.InclusionExclusion;
		else if (runODPinParallelWithIP_radioButton.isSelected())
			input.solverName = SolverNames.ODPinParallelWithIP;
		...
		// Printing options
		if (printDetailedResultsOfIPToFiles_checkBox.isSelected())
			input.printInterimResultsOfIPToFiles = true;
		else
			input.printInterimResultsOfIPToFiles = false;
		if (storeCoalitionValuesToFile_checkBox.isSelected())
			input.storeCoalitionValuesInFile = true;
		else
			input.storeCoalitionValuesInFile = false;
		if (readCoalitionValuesFromFile_checkBox.isSelected())
			input.readCoalitionValuesFromFile = true;
		else
			input.readCoalitionValuesFromFile = false;
		if (printDetailsOfSubspaces_checkBox.isSelected())
			input.printDetailsOfSubspaces = true;
		else
			input.printDetailsOfSubspaces = false;
		if (printTimeTakenByIPForEachSubspace_checkBox.isSelected())
			input.printTimeTakenByIPForEachSubspace = true;
		else
	...
	/**
	 * This method prints the search outcome (e.g. solution value, num of solutions
	 * searched, ...)
	 */
	public void printResultOnGUI(Input input, Result result) {
		...
		if (input.solverName == SolverNames.InclusionExclusion) {
			textArea.append("----------------------------------------------------\n    Inclusion-Exclusion ("
					+ input.numOfAgents + " agents, " + ValueDistribution.toString(input.valueDistribution)
					+ " distribution)\n----------------------------------------------------\n");
		...
		if ((input.solverName == SolverNames.DP) || (input.solverName == SolverNames.IDP)
			...
			textArea.append("----------------------------------------------------\n    "
					...
					+ ValueDistribution.toString(input.valueDistribution)
					+ " distribution)\n----------------------------------------------------\n");
			textArea.append("\nThe run time of " + SolverNames.toString(input.solverName)
					+ " for every size (in milliseconds):\n");
			for (int size = 2; size <= input.numOfAgents; size++)
				textArea.append("    - Size " + size + " took " + result.dpTimeForEachSize[size] + "\n");
			textArea.append("\nThe total run time of " + SolverNames.toString(input.solverName)
					+ " (in milliseconds):\n" + result.dpTime + "\n");
		...
		if ((input.solverName == SolverNames.IP) || (input.solverName == SolverNames.ODPIP)
			...
			if (input.numOfRunningTimes == 1) {
				...
				textArea.append(
						"\nThe value of this coalition structure is:\n" + String.format("%.3f", result.get_ipValueOfBestCSFound()) + "\n");
				if (input.solverName == SolverNames.IP) {
					...
					textArea.append("\nBased on this, the percentage of search-space that was searched by "
							...
							+ (result.ipNumOfExpansions * 100) / result.totalNumOfExpansions + "%\n");
				}
			} else {
				...
				if (input.solverName == SolverNames.IP) {
					textArea.append("\nThe average number of expansions made by "
							...
							+ result.ipNumOfExpansions_confidenceInterval + "\n");
	...
	private void actionPerformed(ActionEvent e) {
		...
		if (e.getActionCommand() == run_button.getActionCommand()) {
			main();
		}
	}
	...
	private void jbInit() throws Exception {
		...
		jLabel3.setText("Problem to solve");
		...
		solveCSGBasedOnUserInput_radioButton
				.addMouseListener(new MainFrame_solveCSGBasedOnUserInput_radioButton_mouseAdapter(this));
		...
		solveCSGBasedOnPredefinedInput_radioButton
				.addMouseListener(new MainFrame_solveCSGBasedOnPredefinedInput_radioButton_mouseAdapter(this));
		...
		runODPinParallelWithIP_radioButton
				.addMouseListener(new MainFrame_runODPinParallelWithIP_radioButton_mouseAdapter(this));
		...
		runInclusionExclusion_radioButton
				.addMouseListener(new MainFrame_runInclusionExclusion_radioButton_mouseAdapter(this));
	...
	public void solveCSGBasedOnUserInput_radioButton_mouseClicked(MouseEvent e) {
		...
		// runCplex_radioButton.setEnabled(true); %runCplex is kept disabled, as the
