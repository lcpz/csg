...
import dp.DPSolver;
import inclusionExclusion.InclusionExclusionSolver;
...
public class MainSolver {
	...
	public Result solve(Input input) {
		ComputeErrorBars computeErrorBars = new ComputeErrorBars(input); // to compute the error when averaging over
																			// multiple runs

		for (int problemID = 1; problemID <= input.numOfRunningTimes; problemID++) // for every problem instance
		{
			input.problemID = String.valueOf(problemID);
			...
			if (input.solverName == SolverNames.InclusionExclusion) {
				InclusionExclusionSolver inclusionExclusionSolver = new InclusionExclusionSolver();
				inclusionExclusionSolver.solve(input, result);
			} else if ((input.solverName == SolverNames.DP) || (input.solverName == SolverNames.IDP)) {
				DPSolver dpSolver = new DPSolver(input, result);
				dpSolver.runDPorIDP();
			...
			if (input.numOfRunningTimes == 1) {
				...
				System.out.println(input.numOfAgents + " agents, " + ValueDistribution.toString(input.valueDistribution)
						+ " distribution. The solver just finished solving " + input.problemID + " problems out of  "
						+ input.numOfRunningTimes);
				if (input.solverName == SolverNames.InclusionExclusion)
					System.out.println(
							"runtime for Inclusion-Exclusion (in milliseconds) was: " + result.inclusionExclusionTime);
			}
		}
