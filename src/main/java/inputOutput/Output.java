...
public class Output {
	...
	public void emptyStringBufferContentsIntoOutputFiles(Input input) {
		if (input.printInterimResultsOfIPToFiles) {
			...
			if (time_numOfRemainingCoalitions.length() != 0) {
				General.printToFile(tempStr + "time_numOfRemainingCoalitions.txt",
						time_numOfRemainingCoalitions.toString(), false);
			...
			if (numOfSearchedCoalitions_solutionQuality.length() != 0) {
				General.printToFile(tempStr + "numOfSearchedCoalitions_solutionQuality.txt",
						numOfSearchedCoalitions_solutionQuality.toString(), false);
				numOfSearchedCoalitions_solutionQuality.setLength(0);
			}
			if (numOfSearchedCoalitions_boundQuality.length() != 0) {
				General.printToFile(tempStr + "numOfSearchedCoalitions_boundQuality.txt",
						numOfSearchedCoalitions_boundQuality.toString(), false);
	...
	public void printCurrentResultsOfIPToStringBuffer_ifPrevResultsAreDifferent(Input input, Result result) {
		if (input.printInterimResultsOfIPToFiles) {
			...
			if ((prevValues_time_solutionQuality[0] != timeThatElapsedSinceIPStarted)
				...
				time_solutionQuality
						.append(timeThatElapsedSinceIPStarted + ", " + result.get_ipValueOfBestCSFound() + "\n");
			...
			if ((prevValues_numOfSearchedCoalitions_solutionQuality[0] != result.ipNumOfExpansions)
				...
				numOfSearchedCoalitions_solutionQuality
						.append(result.ipNumOfExpansions + ", " + result.get_ipValueOfBestCSFound() + "\n");
	...
	public void printFinalResultsOfIPToFiles(Input input, Result result) {
		if (input.printInterimResultsOfIPToFiles) {
			...
			if (result.get_ipBestCSFound() == null)
				...
				levelOfBestCSFoundByIP_asString = String.valueOf(result.get_ipBestCSFound().length);

			// Get the integer partition of the optimal coalition structure, and convert it
			// to string
			...
			if (result.get_ipBestCSFound() == null)
				...
				integerPartitionOfBestCSFoundByIP_asString = General
						.convertArrayToString(integerPartitionOfBestCSFoundByIP);
			}

			// Print the results
			String tempStr = input.outputFolder + "/";
			General.printToFile(tempStr + "bestCSFoundByIP.txt", bestCSFoundByIP_asString + "\n", false);
			General.printToFile(tempStr + "integerPartitionOfBestCSFoundByIP.txt",
					integerPartitionOfBestCSFoundByIP_asString + "\n", false);
			...
			General.printToFile(tempStr + "time_solutionQuality.txt",
					timeThatElapsedSinceIPStarted + ", " + result.get_ipValueOfBestCSFound() + "\n", false);
			General.printToFile(tempStr + "time_boundQuality.txt",
					timeThatElapsedSinceIPStarted + ", " + boundQuality + "\n", false);
			General.printToFile(tempStr + "numOfSearchedCoalitions_boundQuality.txt",
					result.ipNumOfExpansions + ", " + boundQuality + "\n", false);
			General.printToFile(tempStr + "numOfSearchedCoalitions_solutionQuality.txt",
					result.ipNumOfExpansions + ", " + result.get_ipValueOfBestCSFound() + "\n", false);
		}
	}
	...
	public void printDetailsOfSubspaces(Input input, Result result) {
		if (input.printDetailsOfSubspaces) {
			...
			for (int i = 0; i < nodes.length; i++) {
				...
				for (int j = 0; j < nodes[i].length; j++) {
					...
					System.out.println(",   size = " + curSubspace.sizeOfSubspace + " ("
							+ General.setDecimalPrecision(percentagePerSubspace, 5) + "%)," + "  UB = " + curSubspace.UB
							+ ",  LB = " + curSubspace.LB + ",  AVG = " + curSubspace.Avg);
				}
			}
			...
			for (int i = 0; i < nodes.length; i++)
				System.out.println(
						"level" + (i + 1) + ": " + General.setDecimalPrecision(percentagePerLevel[i], 5) + "%\n");
	...
	public void printTimeRequiredForEachSubspace(Subspace subspace, Input input) {
		...
		if (input.printTimeTakenByIPForEachSubspace) {
			...
			System.out.println(
					"Searching " + integerPartitionAsString + " took " + subspace.timeToSearchThisSubspace + " ms.");
