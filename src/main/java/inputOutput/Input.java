...
import model.RandomPartition;

public class Input {
	// The parameters that can only be set here (i.e., not through the GUI):
	public final int maxAllowedCoalitionValue = Integer.MAX_VALUE; // this is usually Integer.MAX_VALUE, but when
																	// testing inclusion-exclusion, we set it to a small
																	// value, e.g., 5
	public final boolean ipUsesBranchAndBound = true; // only relevant when running IP or ODP-IP
	public final boolean ipPruneSubspaces = true; // only relevant when running IP or ODP-IP. Determines whether IP is
													// allowed to prune entire subspaces based on their bounds.
	public final boolean printPercentageOf_v_equals_f = false; // only relevant when using dynamic programming. The
																// percentage of coalitions whose value equals their
																// best partition
	public final boolean printDistributionOfThefTable = false; // only relevant when using dynamic programming. The
																// distribution of the f table
	public final boolean printCoalitionValueDistribution = false; // the distribution of the coalition values
	public final boolean printCoalitionStructureValueDistribution = false; // the distribution of the coalition
																			// structure values (based on a sample of
																			// coalition structures)
	public final boolean printTheSubspaceThatIsCurrentlyBeingSearched = false; // only relevant when running IP or
																				// ODP-IP
	...
	public final boolean useEfficientImplementationOfIDP = true; // if "true", IDP evaluates more splits, but this
																	// allows for a more efficient implementation and a
																	// shorter runtime.
	private final double sigma = 0.1; // relevant to the following distributions: "Normal", "Modified Normal", and
										// "Agent-based Normal"

	public boolean odpipUsesLocalBranchAndBound; // when this is "false", the ODP-IP does not improve branch-and-bound
													// using the technique called "local branch and bound"
	public boolean odpipSearchesMultipleSubspacesSimultaneiously; // when this is "false" and we are running ODP-IP, IP
																	// will not search multiple subspaces simultaneously
	...
	// The IP parameters
	public boolean orderIntegerPartitionsAscendingly; // setting this to false means integers will be ordered
														// descendingly
	...
	public String folderInWhichCoalitionValuesAreStored = "D:/CSGtemp/coalitionValues"; // this is the default setting,
																						// unless it is modified by
																						// another method

	public String problemID = ""; // This is the ID of the problem instance (useful when taking average over
									// multiple instances)

	// ******************************************************************************************************

	/**
	 * Sets some parameters to default values. These values can be changed later on
	 * of course.
	 */
	public void initInput() {
		feasibleCoalitions = null; // this is only needed for a constraint solver. Just ignore it.
		numOfRunningTimes = 1; // Setting this to 1 means we are not taking an average over multiple run with
								// different problem instaces.
		storeCoalitionValuesInFile = false; // Setting this to false means we won't be storing the coalition values
	...
	/**
	 * Return the value of a coalition where every member is represented by an
	 * integer
	 */
	...
	/**
	 * Return the value of a given coalition structure CS, where each coalition
	 * member is represented using an integer
	 */
	...
	/**
	 * Return the value of a given coalition structure CS, where each coalition
	 * member is represented using a bit
	 */
	public double getCoalitionStructureValue(int[] coalitionStructure) {
		double valueOfCS = 0;
		for (int i = 0; i < coalitionStructure.length; i++)
	...
	public void generateCoalitionValues() {
		...
		// Give random strengths to the agents. This is only used in "agent-Based"
		// distributions
		...
		for (int agent = 1; agent <= numOfAgents; agent++) {
			agentStrength_normal[agent - 1] = Math.max(0,
					General.getRandomNumberFromNormalDistribution(10, sigma, random));
		...
		if (valueDistribution == ValueDistribution.UNIFORM)
			for (int coalition = coalitionValues.length - 1; coalition > 0; coalition--) {
				...
				if (maxCoalitionValue < coalitionValues[coalition])
					maxCoalitionValue = coalitionValues[coalition];
			}
		// Normal distribution
		if (valueDistribution == ValueDistribution.NORMAL)
			for (int coalition = coalitionValues.length - 1; coalition > 0; coalition--) {
				coalitionValues[coalition] = Math.max(0,
						Integer.bitCount(coalition) * General.getRandomNumberFromNormalDistribution(10, sigma, random));
		...
		if (valueDistribution == ValueDistribution.MODIFIEDUNIFORM)
			for (int coalition = coalitionValues.length - 1; coalition > 0; coalition--) {
				...
				if (probability <= 20)
					coalitionValues[coalition] += random.nextDouble() * 50;
				coalitionValues[coalition] = Math.round(coalitionValues[coalition] * 10000000);
				if (maxCoalitionValue < coalitionValues[coalition])
					maxCoalitionValue = coalitionValues[coalition];
			}
		...
		if (valueDistribution == ValueDistribution.MODIFIEDNORMAL)
			for (int coalition = coalitionValues.length - 1; coalition > 0; coalition--) {
				...
				if (probability <= 20)
					coalitionValues[coalition] += General.getRandomNumberFromNormalDistribution(25, sigma, random);
				coalitionValues[coalition] = Math.round(coalitionValues[coalition] * 10000000);
				if (maxCoalitionValue < coalitionValues[coalition])
					maxCoalitionValue = coalitionValues[coalition];
			}
		...
		if (valueDistribution == ValueDistribution.EXPONENTIAL) {
			ExponentialDistribution exponentialDistribution = new ExponentialDistribution(1); // the mean of
																											// an
																											// exponential
																											// distribution
																											// is
			...
			for (int coalition = coalitionValues.length - 1; coalition > 0; coalition--) {
				...
				do {
					try {
						coalitionValues[coalition] = Math.max(0,
								Integer.bitCount(coalition) * exponentialDistribution.sample());
		...
		if (valueDistribution == ValueDistribution.BETA) {
			...
			for (int coalition = coalitionValues.length - 1; coalition > 0; coalition--) {
				...
				do {
					try {
						coalitionValues[coalition] = Math.max(0,
								Integer.bitCount(coalition) * betaDistribution.sample());
					} catch (MathRuntimeException e) {
						repeat = true;
					}
				} while (repeat == true);
		...
		if (valueDistribution == ValueDistribution.GAMMA) {
			GammaDistribution gammaDistribution = new GammaDistribution(2, 2);
			for (int coalition = coalitionValues.length - 1; coalition > 0; coalition--) {
				boolean repeat = false;
				do {
					try {
						coalitionValues[coalition] = Math.max(0,
								Integer.bitCount(coalition) * gammaDistribution.sample());
					} catch (MathRuntimeException e) {
						repeat = true;
					}
				} while (repeat == true);
		...
		if (valueDistribution == ValueDistribution.AGENTBASEDUNIFORM)
			for (int coalition = coalitionValues.length - 1; coalition > 0; coalition--) {
				...
				// if percentage is, say 60%, then an agent's value can go up by at most 60%,
				// and down by at most 60%
		...
		if (valueDistribution == ValueDistribution.AGENTBASEDNORMAL)
			for (int coalition = coalitionValues.length - 1; coalition > 0; coalition--) {
				...
				for (int m = 0; m < Integer.bitCount(coalition); m++) {
					double newValue;
					newValue = General.getRandomNumberFromNormalDistribution(agentStrength_normal[members[m] - 1],
							sigma, random);
		...
		if (valueDistribution == ValueDistribution.NDCS)
			for (int coalition = coalitionValues.length - 1; coalition > 0; coalition--) {
				do {
					coalitionValues[coalition] = (General.getRandomNumberFromNormalDistribution(
							Integer.bitCount(coalition), Math.sqrt(Integer.bitCount(coalition)), random));
		...
		if (maxCoalitionValue > maxAllowedCoalitionValue)
			for (int c = coalitionValues.length - 1; c >= 0; c--) // for every coalition, c
				coalitionValues[c] = (int) Math
						.round(coalitionValues[c] * maxAllowedCoalitionValue / maxCoalitionValue);

		System.out.println(numOfAgents + " agents, " + ValueDistribution.toString(valueDistribution)
				+ " distribution. The time required to generate the coalition values (in milli second): "
				+ (System.currentTimeMillis() - time));

		// get the coalition value distribution
		if (printCoalitionValueDistribution)
			printCoalitionValueDistribution();

		// get the coalition structure value distribution
		if (printCoalitionStructureValueDistribution)
			printCoalitionStructureValueDistribution(numOfAgents);
	}
	...
	public void storeCoalitionValuesInFile(int problemID) {
		...
		filePathAndName += "/" + numOfAgents + "Agents_" + ValueDistribution.toString(valueDistribution) + "_"
				+ problemID + ".txt";
	...
	public void readCoalitionValuesFromFile(int problemID) {
		...
		// Set the name and path of the file from which we will read the coalition
		// values
		String filePathAndName = folderInWhichCoalitionValuesAreStored;
		filePathAndName += "/" + numOfAgents + "Agents_" + ValueDistribution.toString(valueDistribution) + "_"
				+ problemID + ".txt";
		try {
			...
			for (int coalition = 0; coalition < coalitionValues.length; coalition++)
				coalitionValues[coalition] = Double.valueOf(bufferedReader.readLine());
	...
	/**
	 * Given the coalition values, print the "weighted" distribution (i.e., where
	 * every coalition value is divided by the size of that coalition).
	 */
	public void printCoalitionValueDistribution() {
		...
		System.out.println(
				"The maximum weighted coalition value (i.e., value of coalition divided by size of that coalition) is  "
						+ max + "  and the minimum one is  " + min);
		...
		// Printing
		System.out.println(
				"The distribution of the weighted coalition values (i.e., every value of coalition is divided by size of that coalition) is:");
	...
	public void printCoalitionStructureValueDistribution(int n) {
		...
		// Printing
		System.out.println(
				"The distribution of the weighted coalition STRUCTURE values  (i.e., every value of coalition is divided by size of that coalition) is:");
