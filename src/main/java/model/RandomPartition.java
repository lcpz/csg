...
public class RandomPartition {
	...
	private void calculateWeights(int n) {
		...
		weightsDouble[0][0] = 1;
		BigDecimal bellNInverse = BigDecimal.ONE.divide(this.getBellNumber(n), 500, RoundingMode.UP);
		for(int i=1; i<=n; ++i) {
		...
		for(int j=n-1; j>=1; --j) {
			for(int k=1; k<=j; ++k) {
				...
				this.probabilityOfNew[j][k] = weights[j+1][k+1].divide(weights[j][k], 500, RoundingMode.UP).doubleValue();
			}
		}
	}
	...
	private BigDecimal[] computeBinomials(int n) {
		...
		for(int i=1; i<=Math.floor(n/2); ++i) {
			binomials[i] = binomials[i-1].multiply(new BigDecimal(n-i+1)).divide(new BigDecimal(i), 100, RoundingMode.UP);
		}
